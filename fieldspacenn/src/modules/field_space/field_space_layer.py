...
def _copy_embeddings_for_layer(
    emb: Optional[Dict[str, Any]],
) -> Optional[Dict[str, Any]]:
    """Copy mappings that field-layer preprocessing may extend or cache into."""
    if emb is None:
        return None

    copied = dict(emb)
    for emb_key in (*_TIME_EMBEDDING_KEYS, GLOBAL_EMBEDDER_CACHE_KEY):
        if emb_key in emb and isinstance(emb[emb_key], Mapping):
            copied[emb_key] = dict(emb[emb_key])
    return copied
...
class FieldSpaceLayerConfig:
    def __init__(
        ...
        **kwargs: Any
        ...
        :param use_indexed_input: Whether the input projection uses indexed parameters.
...
class FieldSpaceLayerBlock(nn.Module):
    ...
    def __init__(
        ...
        pre_shape[1] = (
            in_features_space if embedder and embedder.has_space() else 1
        )
        ...
        self.pre_layer = LinEmbLayer(
            ...
            embedder_cache_key=(
                embedder_cache_key
                if max(self.in_zooms) == input_zoom_field
                else None
            ),
        )
    ...
    def forward(
        ...
        **kwargs: Any
        ...
        emb_for_layer = _copy_embeddings_for_layer(emb)
        if emb_for_layer:
            self.update_time_embedder(emb_for_layer)
        emb_tokenized = emb_for_layer
        if emb_for_layer:
...
class ExtFieldSpaceLayerBlock(FieldSpaceLayerBlock):
    ...
    def __init__(
        ...
        for zoom in self.in_zooms:
            ...
            pre_shape[1] = (
                input_shape[1] if embedder and embedder.has_space() else 1
            )
