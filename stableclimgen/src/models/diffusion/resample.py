...
def create_named_schedule_sampler(name, diffusion_steps):
    ...
    :param diffusion_steps: the diffusion_steps object to sample for.
    """
    if name == "uniform":
...
class ScheduleSampler(ABC):
    """
    A distribution over timesteps in the diffusion_steps process, intended to reduce
    ...
    def weights(self):
        """
        Get a numpy array of weights, one per diffusion_steps step.
...
# Custom linear warm-up scheduler
class LinearWarmUpScheduler(optim.lr_scheduler._LRScheduler):
    def __init__(self, optimizer, warm_up_iters, base_lr, target_lr, last_epoch=-1):
    ...
    def get_lr(self):
        ...
        # Calculate the new learning rate as linear interpolation
        # between base_lr and target_lr over the warm_up_iters
        lr = [
