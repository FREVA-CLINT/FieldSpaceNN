...
class Lightning_MGNO_diffusion_transformer(LightningMGNOBaseModel, LightningProbabilisticModel):
    ...
    def forward(self, x, diffusion_steps, coords_input, coords_output, indices_sample=None, mask=None, emb=None, dists_input=None):
        b, nt, n = x.shape[:3]
        x, mask, emb, _, model_kwargs = self.prepare_inputs(x, coords_input, coords_output, indices_sample, mask, emb, dists_input)
    ...
    def _predict_step(self, source, mask, emb, coords_input, coords_output, indices_sample, input_dists):
        source, mask, emb, interp_x, model_kwargs = self.prepare_inputs(source, coords_input, coords_output, indices_sample, mask, emb, input_dists)
        return self.sampler.sample_loop(self.model, interp_x if torch.is_tensor(interp_x) else source, mask,
