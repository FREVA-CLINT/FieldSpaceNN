...
class Lightning_MGNO_diffusion_transformer(LightningMGNOBaseModel, LightningProbabilisticModel):
    ...
    def forward(self, x, diffusion_steps, coords_input, coords_output, sample_dict={}, mask=None, emb=None, dists_input=None):
        x, mask, emb, model_kwargs = self.prepare_inputs(x, coords_input, coords_output, sample_dict, mask, emb, dists_input)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for i in range(target.shape[0]):
            ...
            in_sample_dict = {k: torch.stack(n_samples*[v[i]]) for k, v in sample_dict.items()}
            in_emb = {k: torch.stack(n_samples*[v[i]]) for k, v in emb.items()}

            l_dict, output = self(in_target, t, in_coords_input, in_coords_output, in_sample_dict, in_mask, in_emb, in_dists_input)

            in_coords_input, in_coords_output, in_sample_dict, in_mask, in_emb, in_dists_input = check_empty(
                in_coords_input), check_empty(
                in_coords_output), check_empty(in_sample_dict), check_empty(in_mask), check_empty(in_emb), check_empty(
                in_dists_input)
            ...
            if batch_idx == 0 and i == 0:
                if hasattr(self, "interpolator") and self.interpolator is not None:
                    input_inter, input_density = self.interpolator(in_source, mask=in_mask, input_coords=in_coords_input,
                                                                   sample_dict=in_sample_dict, calc_density=True,
                                                                   input_dists=in_rel_dists_input)
                else:
                    input_inter = None
                    input_density = None
                has_var = hasattr(self.model, 'predict_var') and self.model.predict_var
                self.log_tensor_plot(in_source, output, in_target, in_coords_input, in_coords_output, in_mask, in_sample_dict,
