...
from ...modules.icon_grids.grid_layer import GridLayer, MultiStepRelativeCoordinateManager, \
    MultiRelativeCoordinateManager
...
from ...modules.neural_operator.no_blocks import PreActivation_NOBlock, NOBlock, Stacked_NOConv, Stacked_NOBlock, \
    Stacked_PreActivationNOBlock, Stacked_PreActivationAttNOBlock
...
class MGNO_EncoderDecoder_Block(nn.Module):
    ...
    def __init__(self,
        ...
        if concat_prev:
            self.indices_concat = [(k, int(torch.where(torch.tensor(input_levels) == global_levels_decode[k])[0])) for k
                                   in range(len(self.model_dims_out)) if global_levels_decode[k] in input_levels]
            self.model_dims_out = [model_dim + input_dims[k] if global_levels_decode[k] in input_levels else model_dim
                                   for k, model_dim in enumerate(self.model_dims_out)]
        ...
        for output_idx, output_level in enumerate(global_levels_decode):
            ...
            for input_idx, input_level in enumerate(input_levels):
                ...
                if level_diff_zero_linear and level_diff == 0:
                    ...
                    embedder = get_embedder(embed_names, embed_confs, embed_mode) if embed_names is not None and len(
                        embed_names) > 0 else None
            ...
            if len(mg_input_dims) > 1:
                if mg_reduction == 'linear':
                    ...
                    if mg_reduction == 'MGAttention':
                        ...
                        reduction_layer = mg.MGDiffMAttentionReductionLayer(
                            torch.tensor(mg_input_levels) - output_level,
                            mg_input_dims,
                            model_dim_out,
                            att_dim=mg_att_dim,
                            n_head_channels=mg_n_head_channels,
        ...
        for output_index, input_indices in enumerate(self.output_indices):
            ...
            for layer_index, input_index in enumerate(input_indices):
                ...
                if isinstance(layer, nn.Identity) or isinstance(layer, nn.Linear):
                    ...
                    x_out, mask_out = layer(x, coords_encode=coords_in, coords_decode=coords_out,
                                            indices_sample=indices_sample, mask=mask, emb=emb)
...
class MGNO_StackedEncoderDecoder_Block(nn.Module):
    ...
    def __init__(self,
        ...
        for input_level in range(int(torch.tensor(input_levels).min()), global_level_no + 1 - no_level_step,
                                 no_level_step):
    ...
    def forward(self, x_levels, coords_in=None, coords_out=None, indices_sample=None, mask_levels=None, emb=None):
        ...
        x_levels_out, mask_levels = self.layer(x_levels, indices_sample=indices_sample, mask_levels=mask_levels,
                                               emb=emb)
