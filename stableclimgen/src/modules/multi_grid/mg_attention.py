...
class Asc_GridCrossAttention(nn.Module):
    ...
    def __init__(self, 
        ...
        for in_zoom_cross in embedders_kv.keys():
            self.kv_layers[in_zoom_cross] = LinEmbLayer(in_features, [2, in_features], layer_confs=layer_confs, identity_if_equal=False, embedder=embedders_kv[in_zoom_cross], layer_norm=True)
...
class Dsc_GridCrossAttention(nn.Module):
