...
def plot_images(gt_images: torch.Tensor, gen_images: torch.Tensor, filename: str, directory: str, xr_dss: Optional[list] = None) -> None:
    ...
    for c in range(gen_images.shape[1]):  # Loop over channels
        subplot_kw = {}
        if xr_dss:
        ...
        fig, axes = plt.subplots(nrows=gt_images.shape[0], ncols=gt_images.shape[2] * 3,
                                 figsize=(2 * img_size * gt_images.shape[2] * 3, img_size * gt_images.shape[0]),
                                 subplot_kw=subplot_kw)
        axes = np.atleast_2d(axes)

        vmin_img = torch.min(gt_images[:, c]).item()
        vmax_img = torch.max(gt_images[:, c]).item()
        vmax_diff = torch.max(torch.abs(differences[:, c])).item()
        vmin_diff = -vmax_diff
        ...
        for i in range(gt_images.shape[0]):  # Loop over samples
            for j in range(gt_images.shape[2]):  # Loop over timesteps
                for index, image, vmin, vmax in [(j, gt_images, vmin_img, vmax_img),
                    ...
                    elif gen_images.dim() == 4:  # Spherical projection for 3D data
                        plt.sca(axes[i, index])
                        pcm = hp.mollview(
                            np.squeeze(image[i, c, j].detach().numpy()), min=vmin, max=vmax,
        ...
        plt.savefig(f'{directory}/{filename}_{c}.png', bbox_inches='tight')
